...
void CInstallFinal()
{
    ...
    while(tmp2)
    {
        tmp2 -> FuncPosition = method_count++;
        tmp2 = tmp2->next;
    }
    class -> Methodcount = method_count;

    if(field_count>8 || method_count>8)
    {
        printf("Error: Class %s has more than 8 fields or functions\n",class->name);
        exit(1);
    }
...
//copy member fields of class_name to current class
void InheritMemberFields(char *class_name)
{
    struct Classtable *ctype = CLookup(class_name);
    if(!ctype)
    {
    ...
    struct Fieldlist *curr = ctype -> Memberfield;
    while(curr)
    {
...
void Class_MethodInstall(char *function_name,struct Typetable *type,struct Paramstruct* plist)
{
    ...
    while(curr)
    {
        if(!strcmp(curr->name,function_name))
        {
            ...
            else    //overriding function by a child class
            {
                curr -> Flabel = get_label();

                //validate function's return type and paramlist is same as parent
                if(curr->type != type)
                {
                    printf("Error: Types of func %s in child and parent differ\n",function_name);
                    yyerror("");
                    exit(1);
                }

                //validate parameter list
                struct Paramstruct *pcurr1 = plist;     //paramlist of function in child
                struct Paramstruct *pcurr2 = curr -> paramlist; //paramlist of function in parent
                while(pcurr1 && pcurr2)
                {
                    if(pcurr1 -> type != pcurr2 -> type)
                    {
                        printf("Error: Types of param in %s in child and parent differ\n",function_name);
                        yyerror("");
                        exit(1);
                    }
                    pcurr1 = pcurr1 -> next;
                ...
                if(pcurr1 || pcurr2)
                {
                    printf("Error: No. of param in func %s in child and parent differ\n",function_name);
                    yyerror("");
                    exit(1);
                }
...
//copy functions of class_name to current class
void InheritFunctions(char *class_name)
{
    struct Classtable *ctype = CLookup(class_name);
    if(!ctype)
    {
    ...
    struct Memberfunclist *curr = ctype -> Vfuncptr;
    while(curr)
    {
        struct Memberfunclist *tmp = (struct Memberfunclist*)malloc(sizeof(struct Memberfunclist));
        tmp -> name = (char*)malloc(sizeof(curr->name));
        strcpy(tmp -> name,curr -> name);
        tmp -> type = curr -> type;
        tmp -> paramlist = curr -> paramlist;
        tmp -> Flabel = curr -> Flabel;
        tmp -> next = NULL;
